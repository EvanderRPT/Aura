...
void UInv_InventoryGrid::UpdateTileParameters(const FVector2D& CanvasPosition, const FVector2D& MousePosition)
{
	...
	const FIntPoint HoverTileCoordinates = CalculateHoveredCoordinates(CanvasPosition, MousePosition);

	LastTileParameters = TileParameters;
	TileParameters.TileCoordinates = HoverTileCoordinates;
	TileParameters.TileIndex = UInv_WidgetUtils::GetIndexFromPosition(HoverTileCoordinates, Columns);
...
{
	return  FIntPoint {
		static_cast<int>(FMath::FloorToInt((MousePosition.X - CanvasPosition.X) / TileSize)),
		static_cast<int>(FMath::FloorToInt((MousePosition.Y - CanvasPosition.Y) / TileSize))
	};
}
...
FInv_SlotAvailabilityResult UInv_InventoryGrid::HasRoomForItem(const FInv_ItemManifest& Manifest, const int32 StackAmountOverride)
{
	...
	for (const auto& GridSlot : GridSlots)
	{
		...
		// Cna the item fit here? (i.e. is it out of grid bounds?)
	    ...
	    const int32 MaxStackSize ) const
{
	...
	// I this Grid slot upper left slot?
...
int32 UInv_InventoryGrid::GetStackAmount(const UInv_GridSlot* GridSlot) const
{
	...
	// If we are at a slot that doesn't hold the stack count. we must get that actual stack count.
...
void UInv_InventoryGrid::AddStacks(const FInv_SlotAvailabilityResult& Result)
{
	...
	for (const auto& Availability : Result.SlotAvailabilities)
	{
		...
		else
		{
			...
			UpdatedGridSlots(Result.Item.Get(), Availability.Index, Result.bStackable, Result.TotalRoomToFill);
		}
	}
}
...
void UInv_InventoryGrid::OnSlottedItemClicked(int32 GridIndex, const FPointerEvent& MouseEvent)
{
	...
	if (IsSameStackable(ClickedInventoryItem))
	{
		...
		const FInv_StackableFragment* StackableFragment = ClickedInventoryItem->GetItemManifest().GetFragmentByType<FInv_StackableFragment>();
...
void UInv_InventoryGrid::CreateItemPopUp(const int32 GridIndex)
{
	...
	if (RightClickedItem->IsConsumable())
	{
		...
		ItemPopUp->ShowConsumableButton();
...
void UInv_InventoryGrid::AddItemToIndices(const FInv_SlotAvailabilityResult& Result, UInv_InventoryItem* NewItem)
{
	...
	for (const auto& Availability : Result.SlotAvailabilities)
	{
		...
		UpdatedGridSlots(NewItem, Availability.Index, Result.bStackable, Availability.AmountToFill);
	}
	...
	const int32 StackAmount)
{
	...
	AddSlottedItemCanvas(Index, GridFragment, SlottedItem);
...
void UInv_InventoryGrid::AddSlottedItemCanvas(const int32 Index, const FInv_GridFragment* GridFragment,
...
void UInv_InventoryGrid::UpdatedGridSlots(UInv_InventoryItem* NewItem, const int32 Index, bool bStackableItem, const int32 StackAmount)
{
...
void UInv_InventoryGrid::PutDownOnIndex(const int32 Index)
{
	...
	UpdatedGridSlots(HoverItem->GetInventoryItem(), Index, HoverItem->IsStackable(), HoverItem->GetStackCount());
	ClearHoverItem();
...
void UInv_InventoryGrid::SwapWithHoverItem(UInv_InventoryItem* ClickedInventoryItem, const int32 GridIndex)
{
	...
	UpdatedGridSlots(TempInventoryItem, ItemDropIndex, bTempIsStackable, TempStackCount);
}
	...
	const int32 Index)
{
	...
	const FInv_GridFragment* GridFragment = GridSlots[Index]->GetInventoryItem()->GetItemManifest().GetFragmentByType<FInv_GridFragment>();
	const FIntPoint Dimensions = GridFragment ? GridFragment->GetGridSize() : FIntPoint(1, 1);
...
void UInv_InventoryGrid::FillInStack(const int32 FillAmount, const int32 Remainder, const int32 Index)
{
	...
	HoverItem->UpdateStackCount(NewStackCount);
...
void UInv_InventoryGrid::SetOwningCanvas(UCanvasPanel* NewOwningCanvas)
{
	OwningCanvasPanel = NewOwningCanvas;
}
...
void UInv_InventoryGrid::OnGridSlotHovered(int32 GridIndex, const FPointerEvent& MouseEvent)
{
	...
	if  (UInv_GridSlot* GridSlot = GridSlots[GridIndex]; GridSlot->IsAvailable())
	{
		GridSlot->SetOccupiedTexture();
	}
}
